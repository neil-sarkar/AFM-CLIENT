    ...
    void Scanner::initialize_scan_state_machine() {
        ...
        fwd_offset_data = new ScanData(m_num_columns, m_num_rows, m_ratio, get_delta_x_from_ratio(), get_delta_y_from_ratio(), true);
        fwd_phase_data = new ScanData(m_num_columns, m_num_rows, m_ratio, get_delta_x_from_ratio(), get_delta_y_from_ratio(), true);
        fwd_error_data = new ScanData(m_num_columns, m_num_rows, m_ratio, get_delta_x_from_ratio(), get_delta_y_from_ratio(), true);
        rev_offset_data = new ScanData(m_num_columns, m_num_rows, m_ratio, get_delta_x_from_ratio(), get_delta_y_from_ratio(), false);
        rev_phase_data = new ScanData(m_num_columns, m_num_rows, m_ratio, get_delta_x_from_ratio(), get_delta_y_from_ratio(), false);
        rev_error_data = new ScanData(m_num_columns, m_num_rows, m_ratio, get_delta_x_from_ratio(), get_delta_y_from_ratio(), false);
    }
    ...
    void Scanner::receive_data() {
        ...
        if (!(fwd_offset_data->is_full() && rev_offset_data->is_full())) {
             cmd_step_scan();
        }
        else {
    ...
    void Scanner::callback_step_scan(QByteArray payload) {
        ...
        for (int i = 0; i < payload.size(); i += 6) { // 6 bytes passed back
            ...
            if (scanning_forward) {
                ...
                rev_error_data->append(pid->setpoint() / ADC::SCALE_FACTOR - z_amplitude);
            }
